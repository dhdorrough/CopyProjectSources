...
{*}   procedure ReadState(Reader: TReader); override;
  ...
  TForm_UsedUnits = class(TForm)
    ...
    ListFilenamesAndPaths1: TMenuItem;
    Utilities1: TMenuItem;
    CopyProjectSources1: TMenuItem;
    ...
    procedure ListFilenamesAndPaths1Click(Sender: TObject);
    procedure CopyProjectSources1Click(Sender: TObject);
  private
    ...
    procedure ListFilenamesandPaths(Lfn: string);
    procedure DumpUnits(Title: string; DoAppend: boolean);
  protected
...
uses StStrL, uAbout, MyUtils, Registry, PathConversions;
//uses StStrL, uAbout;
...
const
  DELPHI7 = 'Software\Borland\Delphi\7.0\';
var
 Registry: TRegistry;
 Path: string;
begin { LoadRegistryPaths }
 ...
 try
   ...
   if not Registry.KeyExists(DELPHI7) then
     begin
       AlertFmt('Key does not exist: %s', [DELPHI7]);
       Exit;
     end;
   Registry.OpenKey(DELPHI7, False);  // False because we do not want to create it if it doesn't exist
   DelphiDir := Registry.ReadString('RootDir');
   if DelphiDir[Length(DelphiDir)] = '\' then
     Delete(DelphiDir, Length(DelphiDir), 1);

   Registry.OpenKey('Library', false);
   Path := Registry.ReadString('Search Path');
   AddPaths(Path);

   Path := Registry.ReadString('Browsing Path');
   AddPaths(Path);
   Registry.CloseKey;
 finally
   Registry.Free;
        ...
        begin
          ...
          with UnitInfo do
            begin
              if (i mod 10) = 0 then
                begin
                  lblNowProcessing.Caption := Format('Processing %d/%d [%s]',
...
procedure TForm_UsedUnits.DumpUnits(Title: string; DoAppend: boolean);
const
  C0 = 4;
  C1 = 25;
  C2 = 50;
...
var
  ...
  OutFile: TextFile;
begin
  AssignFile(OutFile, 'c:\temp\UnitList.txt');
  if DoAppend then
    Append(OutFile)
  else
    ReWrite(OutFile);
  ...
  try
    for i := 0 to lbUnits.Count-1 do
      begin
        ...
        WriteLN(OutFile, i:C0, ' ',
                         Padr(UnitInfo.UnitName, C1), ' ',
                         Padr(UnitInfo.FilePath, C2), ' ',
                         IIF(UnitInfo.Found, 'T', 'F'):C3,  ' ',
                         UnitInfo.UnitsReferenced.Count:C4);

      end;
  finally
    WriteLn(OutFile, Padr('', 100, '-'));
    CloseFile(OutFile);
  end;
end;
  ...
  try
    ...
    try
      ...
      repeat
        ScanFile(UnitNr);
//      if UnitNr = 0 then
//        DumpUnits('After scanning '+Edit_MainProgram.Text, false);
        inc(UnitNr);
      until UnitNr >= lbUnits.Items.Count;
      lbUnits.Sorted := true;
//    DumpUnits('Sorted', true);
      ...
      lblNowProcessing.Caption := Format('FINISHED. Found=%d, Scanned=%d',
...
begin { TForm_UsedUnits.OpenProjectFile }
...
end;  { TForm_UsedUnits.OpenProjectFile }
...
begin { SkipComment }
  ...
  if ch = '(' then
    begin
      if (Idx <= Length(Line)) and (Line[idx] = '*') then
        begin
          NextCh;
          done := false;
          while not done do
            begin
              NextCh;
              if (ch = '*') and (Idx <= Length(Line)) and (Line[Idx] = ')') then
                begin
                  NextCh;
                  done := true;
                end;
...
procedure TForm_UsedUnits.ScanFile(lbIndex: integer);
  ...
  var
    ...
    procedure ProcessInClause;
      var
        FilePath, aCurrentDirectory: string;
        UnitInfo: TUnitInfo;
        idx: integer;
        aUnitName: string;
        FileName: string;
    begin { ProcessInClause }
      ...
      if Empty(FilePath) then
        ...
        begin
          aCurrentDirectory := ExtractFilePath(Edit_MainProgram.Text);
          FilePath := aCurrentDirectory + FilePath;
        end;
      aUnitName := UnitName;
      if pos('.', aUnitName) = 0 then
        aUnitName := aUnitName + '.pas';
      idx := lbUnits.Items.IndexOf(aUnitName);
      if idx >= 0 then
        begin
          UnitInfo := TUnitInfo(lbUnits.Items.Objects[idx]);
          if Assigned(UnitInfo) then
            begin
              UnitInfo.FilePath := FilePath;
              FileName          := FilePath + UnitInfo.UnitName;
              UnitInfo.Found    := FileExists(FileName);
            end;
        end;
      SkipBlanks;
      SkipComment;
      ...
      if not Empty(Token) then
        if CompareText(Token, 'in') = 0 then
          ...
          if ch in [',', ';', 'i', 'I'] then
            begin
              UnitName := Token;
              AddUsedUnit(UnitName)
            end
          else
            exit;  // if the token isn't followed by a comma or a semicolon
                    // or an 'i' (from 'in')
                    // the syntax dosen't make sense and we are probably
...
begin { TForm_UsedUnits.ScanFile }
  ...
  UseClauseCount  := 0;
  if CurrentUnitInfo.Found or FileFound(InLfn, FullName) then
    begin
      if not CurrentUnitInfo.Found then // if so, not already set
        CurrentUnitInfo.FilePath  := ExtractFilePath(FullName)
      else
        FullName := CurrentUnitInfo.FilePath + CurrentUnitInfo.UnitName;

      BaseName                 := ChangeFileExt(ExtractFileName(FullName), '');
      
      lblNowProcessing.Caption := Format('%d: %s', [lbUnits.Items.Count+1, InLfn]);
...
begin
  ...
  with lbUnits do
    begin
      ...
      lblNowProcessing.Caption := CurrentUnitInfo.FilePath + Items[ItemIndex];
    end;
end;
...
//          UnitInfo := TUnitInfo(lbUnits.Items.Objects[UnitNr]); { dosen't work here? }
      ...
      lblNowProcessing.Caption := 'Relation Found';
      Enable_Stuff;
    end
  else
    begin
...
procedure TForm_UsedUnits.ListFilenamesandPaths(Lfn: string);
var
  i: integer;
  UnitInfo: TUnitInfo;
  OutFile: TextFile;
  FullName: string;
  LineCount: integer;
begin { TForm_UsedUnits.ListFilenamesandPaths }
  AssignFile(OutFile, lfn);
  Rewrite(OutFile);
  LineCount := 0;
  try
    for i := 0 to lbUnits.Items.Count-1 do
      begin
        ...
        if Assigned(UnitInfo) then
          with UnitInfo do
            begin
              FullName := UnitInfo.FilePath + UnitInfo.UnitName;
              Write(OutFile, Quoted(FullName));
              Writeln(Outfile);
              Inc(LineCount);
            end;
      end;
    ...
    CloseFile(OutFile);
    lblNowProcessing.Caption := Format('%d/%d lines written to %s',
                                       [LineCount, lbUnits.Items.Count, Lfn]);
  end;
end;

procedure TForm_UsedUnits.ListFilenamesAndPaths1Click(Sender: TObject);
begin
  with SaveDialog1 do
    begin
      InitialDir := ExtractFilePath(Edit_MainProgram.Text);
      FileName   := JustNameL(Edit_MainProgram.Text) + '.csv';
      Filter     := 'Comma Separated Variables (*.csv)|csv';
      DefaultExt := 'csv';
      if Execute then
        ListFilenamesAndPaths(FileName);
    end;
end;

procedure TForm_UsedUnits.CopyProjectSources1Click(Sender: TObject);
(*
const
  Lfn = 'c:\Temp\LogFile.txt';
*)
...
var
  ...
  LineCount, i: integer;
  UnitInfo: TUnitInfo;
  FullName, SrcFilePath, DstFilePath: string;
  DelphiSources, LeadingPath, BasePath, RelativePath: string;
  RootPath, Temp, Lfn: string;
  LogFile: TextFile;
  OkToOverWrite: boolean;
begin
  DestinationFolder := 'C:\Temp';
  if BrowseForFolder('Destination Folder', DestinationFolder) then
    begin
      OkToOverWrite := Yes('Is it OK to overwrite existing files?');
      DelphiSources := ForceBackslash(DelphiDir) + 'source';
      BasePath      := ExtractFilePath(Edit_MainProgram.Text);
      LineCount := 0;

      // try to find leading path common to all source files
      for i := 0 to lbUnits.Items.Count-1 do
        begin
          UnitInfo := TUnitInfo(lbUnits.items.objects[i]);
          if Assigned(UnitInfo) then
            with UnitInfo do
              begin
                LeadingPath := Copy(FilePath, 1, Length(DelphiSources));
                if Found and (not SameText(DelphiSources, LeadingPath)) then
                  begin
                    Inc(LineCount);
                    if RootPath = '' then
                      RootPath := ExpandFileName(UnitInfo.FilePath);  // 1st pass through
                    temp := Copy(UnitInfo.FilePath, 1, Length(RootPath));
                    if not SameText(temp, RootPath) then
                      RootPath := ExtractFilePath(RemoveTrailingBackSlash(RootPath))
                  end;
              end;
        end;
      ...
      if LineCount > 0 then
        begin
          Lfn := DestinationFolder + UniqueFileName('Delphi Sources List.txt');
          AssignFile(LogFile, Lfn);
          ReWrite(LogFile);
          WriteLn(LogFile, 'Source files copied to: ', DestinationFolder);
          WriteLn(LogFile, '    Date/Time: ', DateTimeToStr(Now));
          WriteLn(LogFile);
          try
            LineCount := 0;
            for i := 0 to lbUnits.Items.Count-1 do
              begin
                ...
                if Assigned(UnitInfo) then
                  with UnitInfo do
                    begin
                      LeadingPath := Copy(UnitInfo.FilePath, 1, Length(DelphiSources));
                      if Found and (not SameText(DelphiSources, LeadingPath)) then
                        begin
                          FullName      := ExpandFileName(UnitInfo.FilePath + UnitInfo.UnitName);
                          Inc(LineCount);
                          lblNowProcessing.Caption := Format('Processing %s (%d/%d)',
                                                             [FullName, LineCount, lbUnits.Items.count]);
                          SrcFilePath   := FullName;
                          RelativePath  := ExtractRelativePath(RootPath, FullName);
                          DstFilePath   := RelToAbs(RelativePath, DestinationFolder);
                          if ForceDirectories(ExtractFilePath(DstFilePath)) then
                            if not CopyFile(pChar(SrcFilePath), pChar(DstFilePath), not OkToOverWrite) then
                              Writeln(LogFile, Format('Copy Failed [%s]: %s', [SysErrorMessage(GetLastError), DstFilePath]))
                            else
                              begin
                                WriteLn(LogFile, Format('Copied:           %s', [SrcFilePath]));
                                SrcFilePath := ForceExtension(SrcFilePath, 'dfm');
                                if FileExists(SrcFilePath) then
                                  begin
                                    DstFilePath := ForceExtension(DstFilePath, 'dfm');
                                    if not CopyFile(pChar(SrcFilePath), pChar(DstFilePath), not OkToOverWrite) then
                                      Writeln(LogFile, Format('Copy Failed [%s]: %s', [SysErrorMessage(GetLastError), DstFilePath]))
                                    else
                                      WriteLn(LogFile, Format('Copied:           %s', [SrcFilePath]));
                            ...
                            WriteLn(LogFile, Format('Unable to create directories: %s', [DstFilePath]));
                        end;
                    end;
              end;
            lblNowProcessing.Caption := Format('COMPLETE. Processed (%d/%d)',
                                               [LineCount, lbUnits.Items.count]);
          finally
            CloseFile(LogFile);
